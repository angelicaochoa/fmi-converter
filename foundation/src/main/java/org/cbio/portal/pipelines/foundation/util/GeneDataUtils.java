...
public class GeneDataUtils {
    ...
    private final String HOMO_SAPIENS_GENE_RESOURCE = "Homo_sapiens.gene_info";
    ...
    private void loadHumanGeneInfo() throws Exception {
        // init tab-delim tokenizer and line mapper
        ...
        reader.setResource(new ClassPathResource(HOMO_SAPIENS_GENE_RESOURCE));
        ...
        // read through each line to fill symbol/id maps and ncRNA symbol list
        ...
        while (record != null) {
            ...
            if (record.getProperty(TYPE_OF_GENE_COLUMN).equals("ncRNA")) {
            ...
            if (!record.getProperty(SYNONYMS_COLUMN).equals("-")) {
                String[] synonyms = record.getProperty(SYNONYMS_COLUMN).split("|");
                for (String synonym : synonyms) {
                    geneAliasMap.put(synonym, hugoSymbol);
                }
            }
            record = reader.read();
        }
    ...
    public boolean isNcRNA(String hugoSymbol) {
        return (ncRNABlackList.contains(hugoSymbol) || 
                Strings.isNullOrEmpty(hugoSymbol) || 
                FoundationUtils.NULL_EMPTY_VALUES.contains(hugoSymbol));
    }
    ...
    public String resolveGeneSymbol(String hugoSymbol) {
        if (Strings.isNullOrEmpty(hugoSymbol) || FoundationUtils.NULL_EMPTY_VALUES.contains(hugoSymbol)) {
            return hugoSymbol;
        }
